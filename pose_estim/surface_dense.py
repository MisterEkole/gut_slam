...
class BMeshDeformation:
    def __init__(self, height, center, num_height_points, num_angle_points, num_radius_points):
        self.height = height
        self.center = center
        self.num_height_points = num_height_points
        self.num_angle_points = num_angle_points
        self.num_radius_points = num_radius_points

        self.cylinder_points = self._generate_cylinder_points()

    def _generate_cylinder_points(self):
        heights = np.linspace(0, self.height, self.num_height_points)
        angles = np.linspace(0, 2 * np.pi, self.num_angle_points, endpoint=True)
        radii = np.linspace(0, 500, self.num_radius_points)
...
# Define the cylinder with higher resolution
height = 1000
center = (0, 0)
num_height_points = 10
num_angle_points = 10
num_radius_points = 10

bmd = BMeshDeformation(height, center, num_height_points, num_angle_points, num_radius_points)

# Generate a uniform grid of control points
rho_step_size = 25  # Smaller step size for rho
alpha_step_size = np.pi / 10  # Smaller step size for alpha

# Set a constant height
h_constant = 100
control_points = generate_uniform_grid_control_points(rho_step_size, alpha_step_size, h_constant=10)

# Increase the resolution of B-spline evaluation
a = 1
b = 1
deformed_points = bmd.b_mesh_deformation(a, b, control_points)
